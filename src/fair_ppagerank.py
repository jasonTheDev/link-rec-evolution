...
class PLFPRfromPaper:
    ...
    def compute_Lr_Lb(self):
        outC1, outC2 = self.outC1,self.outC2
        ratioC1 = [outC1[i] / (outC1[i]+outC2[i]) for i in range(len(outC1))]
        Lr=set()
        Lb=set()
    ...
    def process(self):
        ...
        for a in range(0,self.nnode):
            ...
            while(iteration<100):
                #+self.alpha*(v)
                #pr = (1-self.alpha)*(np.dot(pr_init,(self.PL+X_P+Y_P)))+v
